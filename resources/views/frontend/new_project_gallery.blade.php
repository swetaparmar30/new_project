            ...
            <div class="row popup-gallery details-tab-details" id="all">
                @if (isset($photos) && count($photos) > 0 && !empty($photos))
                    @foreach ($photos as $key => $val)
                        ...
                        @endif
                        <div
                            class="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-12 col-sm-12 text-center each-image @if ($key > 2)  @endif">
                            <figure>
                                <div class="gallery-item">
                                    <a data-effect="mfp-zoom-in"
                                        href="{{ isset($img_2->name) ? asset('uploads/' . $img_2->name) : (isset($img->name) ? asset('uploads/' . $img->name) : '#') }}"
                                        title="" class="a">
                                        <img class="img-fluid" src="{{ asset('uploads/' . $img->name) }}"
                                            alt="{{ $val->title }}" />
                                        <img class="plus-img"
                                            src="{{ asset('front-assets/src/images/white-zoome-img-icon.svg') }}"
                                            width="80" height="80">
                                    </a>
                                </div>
                            </figure>
                    ...
                    <div class="row popup-gallery details-tab-details" id="{{ $cat->slug }}">
                        @if (isset($images) && count($images) > 0 && !empty($images))
                            @foreach ($images->take(15) as $key => $val)
                                ...
                                @endif
                                <div
                                    class="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-12 col-sm-12 each-image text-center  @if ($key > 2)  @endif">
                                    <figure>
                                        <div class="gallery-item">
                                            <a data-effect="mfp-zoom-in"
                                                href="{{ isset($img_2->name) ? asset('uploads/' . $img_2->name) : (isset($img->name) ? asset('uploads/' . $img->name) : '#') }}"
                            ...
                            @if (isset($images) && count($images) > 15 && !empty($images))
                                <div>
                                    <a class="common-btn text-center show_less" id="show_less"
                                        data-container="{{ $cat->slug }}-remaining" style="cursor: pointer;">Show
                                        Less</a>
                                </div>
                            @endif
                        @else
